...
@PIPELINES.register_module()
class NormalizeList(Normalize):  #chgd
    """Normalize the image.
            ...
            for index_ in range(len(results[key])):
                if self.size is not None:
                    ...
                    padded_img = mmcv.impad_to_multiple(
                        results[key][index_], self.size_divisor, pad_val=self.pad_val)
                results[key][index_] = padded_img
...
class RandomFlip3DList(RandomFlip3D):
    ...
    def __call__(self, input_dict):
        ...
        """
        # flip 2D image and its annotations
        super(RandomFlip3DList, self).__call__(input_dict)
